...
5.  Now, calculate the power produced by a wind farm for a given wind direction and free stream windspeed.  Assume your turbines have a cut-in speed of 3 m/s, a rated wind speed of 11.4 m/s, a rated power of 5 MW, and a rotor diameter of 126.4 m.  To be simple, assume that the power produced by a turbine is only dependent on the wind speed at the turbine hub.  To be more complex, calculate the effective velocity at a turbine as an area weighted average of the wind speeds across the rotor.
6.  Finally, modify your code to calculate annual energy production (AEP). Given a set of wind directions with their associated probabilities and wind speeds, modify/add to your code to calculate the amount of energy produced by a wind farm throughout the year.
