...
p1 = [0.0 , 0.4]   #First p value, Maximum Camber
p2 = [5.0, 8.0]   #Second p value, percent location of max camber along chord
...
for i in range(int((p1[1]-p1[0])/h)):
    for j in range(int((p2[1]-p2[0])/h)):
        for k in range(int((p3[1]-p3[0])/h)):
            ...
            data = temp
            percent = 100*(i*j*k)/((int((p1[1]-p1[0])/h))*(int((p2[1]-p2[0])/h))*(int((p3[1]-p3[0])/h)))
            print("Percent done"),
            print(percent)
